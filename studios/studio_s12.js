...
function is_unary_function(candidate) {
  return (
    is_compound_function(candidate) &&
    length(function_parameters(candidate)) === 1
  );
}

const make_composed_function = evaluate(
  parse("(f, g) => x => f(g(x));"),
  the_empty_environment
);

const overloaded_add = (x, y) =>
  is_number(x) && is_number(y)
    ? x + y
    : is_unary_function(x) && is_unary_function(y)
    ? apply(make_composed_function, list(x, y))
    : error("expected functions or numbers");
...
// POINT
function set_name_value(name, boo, val, env) {
  function scan(names, vals) {
    if (is_null(names)) {
      return error("internal error: name not found");
    } else if (name === head(names)) {
      set_tail(head(vals), boo);
      return set_head(head(vals), val);
    } else {
      return scan(tail(names), tail(vals));
    }
  }
  // function scan(names, vals) {
  //     return is_null(names)
  ...
  function env_loop(env) {
    function scan(names, vals) {
      return is_null(names)
        ...
        ? tail(head(vals))
          ? set_head(head(vals), val)
          : error("no assignment " + "to constants allowed bitch")
        : scan(tail(names), tail(vals));
    }
    if (is_empty_environment(env)) {
      error(name, "Unbound name in assignment: ");
...
function setup_environment() {
  const primitive_function_names = map(f => head(f), primitive_functions);
  // display(primitive_function_names);
  const primitive_function_values = map(
    f => make_primitive_function(head(tail(f))),
    primitive_functions
  );
...
display(parse_and_eval("function f(x) {x = 2;} f(2);"));
...
function eval_block(stmt, env) {
  ...
  return evaluate(body, block_env);
}
...
// read_eval_print_loop("");
parse_and_eval("function f(x) {return x;}");
parse_and_eval("function y(f) {return x => f(x);}");
parse_and_eval("f(2);");
