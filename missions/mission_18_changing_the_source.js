...
/*
Evaluator for language with numbers, booleans, 
conditionals, sequences, functions, constants, 
variables and blocks and lists, including the 
higher-order functions map, accumulate and filter.

The covered Source §3 sublanguage is:
...
function is_self_evaluating(stmt) {
  return (
    is_number(stmt) || is_string(stmt) || is_boolean(stmt) || is_null(stmt)
  );
}
...
// MODIFIED: Add overloaded add function
function add(x, y) {
...
// MODIFIED: Add negation operation
function minus(x, y) {
  if (y === undefined) {
    return -x;
  } else {
...
const primitive_functions = list(
  ...
  list("error", error),
  // MODIFIED: change to overloaded add function
  list("+", add),
  // MODIFIED: Add negation operation
  list("-", (x, y) => minus(x, y)),
  list("*", (x, y) => x * y),
...
function parse_and_eval(str) {
  return eval_toplevel(parse(prelude + " { " + str + " } "));
}

// Test
parse_and_eval("display(list(1, 2, 3) + list(4, 5, 6) + list(7, 8, 9));");

// examples:
// parse_and_eval("display(1);");
// parse_and_eval("display(1 + 1);");
// parse_and_eval("display(1 + 3 * 4);");
// parse_and_eval("display((1 + 3) * 4);");
// parse_and_eval("display(1.4 / 2.3 + 70.4 * 18.3);");

// parse_and_eval("display(true);");
// parse_and_eval("display(! (1 === 1));");
// parse_and_eval("display((! (1 === 1)) ? 1 : 2);");

// parse_and_eval("display('hello' + ' ' + 'world');");
...
// parse_and_eval("function length(xs) {          " +
//               "    return is_null(xs)         " +
//               "        ? 0                    " +
//               "        : 1 + length(tail(xs));" +
//               "}                              " +
//               "display(length(list(10, 20, 30)));      "   );

// parse_and_eval("display(map(x => x + 1, list(1, 2, 3))); ");

// parse_and_eval("display(filter(x => x % 2 === 0,       " +
//               "       list(1, 2, 3, 4, 5, 6)));"   );

// parse_and_eval("display(accumulate((x, y) => x + y,    " +
//               "   0, list(10, 20, 30)));       "   );
...
// MODIFIED: add global variable
let curr_environment = null;
...
// MODIFIED: Add overloaded ">" function
function map_alternative(x, y) {
  if (is_list(y)) {
    return apply(lookup_name_value("map", curr_environment), list(x, y));
  } else {
    return x > y;
  }
}
...
const program_string =
  ...
  "return x * 10;" +
  "}" +
  "display(multiply_by_ten > list(1, 2, 3));"; // returns list(10, 20, 30);

("display(math_abs > list(5, -10, 15, 20, -25));"); // returns list(5, 10, 15, 20, 25);

("display((x => x * x) > list(1, 2, 3));"); // returns list(1, 4, 9);
...
function apply(fun, args) {
  if (is_primitive_function(fun)) {
    ...
    if (length(args) < length(function_parameters(fun))) {
      let completed_params = take(function_parameters(fun), length(args));

      let left_params = drop(function_parameters(fun), length(args));

      let new_env = extend_environment(
        completed_params,
        args,
        function_environment(fun)
      );

      result = make_compound_function(left_params, function_body(fun), new_env);
    } else {
      const body = function_body(fun);
      const locals = local_names(body);
      const names = insert_all(function_parameters(fun), locals);
    ...
    if (is_compound_function(result)) {
      return result;
...
// MODIFIED: evaluate compound functions
function evaluate(stmt, env) {
  // display("evaluating: ", stmt);
  return is_self_evaluating(stmt)
    ? stmt
    : is_name(stmt)
    ? lookup_name_value(name_of_name(stmt), env)
...
parse_and_eval("map(x => x + 1, list(1, 2, 3); ";

parse_and_eval("filter(x => x % 2 === 0,       " +
               "       list(1, 2, 3, 4, 5, 6));"   );

parse_and_eval("accumulate((x, y) => x + y,    " +
               "   0, list(10, 20, 30));       "   );
...
// Tests
// const program_string =
// "function g(a1, a2, a3, a4, a5, a6) {" +
//     "return a1 + a2 + a3 + a4 + a5 + a6;" +
// "}" +
// "const g1 = g(1, 2);" + // returns function that takes a3, a4, a5, a6
// "const g2 = g1(3, 4, 5);" + // returns function that takes a6
// "const g3 = g2(6);" +
// "g3;"; // returns 21

// "const even_filter = filter(x => x % 2 === 0);"   +
// "const list_flattener = accumulate(append, null);" +
// "const list_squarer = map(x => x * x);" +

// "even_filter(list(1, 2, 3, 4));" +// returns list(2, 4)
// "list_flattener(list(list(1, 2), list(3), list(4, 5)));" +
//                         // returns list(1, 2, 3, 4, 5)
// "list_squarer(list(1, 2, 3));";  // returns list(1, 4, 9)
